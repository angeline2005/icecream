...
export const Customize = ({ movable }) => {
  ...
  const [iceCreamCategories, setIceCreamCategories] = useState([]);

  

  useEffect(() => {
    fetch("http://localhost:5000/api/customizes")
      .then((response) => response.json())
      .then((data) => {
        setIceCreamCategories(data);
        console.log(iceCreamCategories);
      })
      .catch((error) => console.error("Error fetching data:", error));
  }, []);

  const [categories, setCategories] = useState(iceCreamCategories);
